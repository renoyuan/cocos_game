    ...
    execute: function () {
      ...
      _export("GameModel", GameModel = class GameModel {
        ...
        init(cellTypeNum) {
          ...
          for (var _i = 1; _i <= (_crd && GRID_WIDTH === void 0 ? (_reportPossibleCrUseOfGRID_WIDTH({
            error: Error()
          }), GRID_WIDTH) : GRID_WIDTH); _i++) {
            for (var _j = 1; _j <= (_crd && GRID_HEIGHT === void 0 ? (_reportPossibleCrUseOfGRID_HEIGHT({
              error: Error()
            }), GRID_HEIGHT) : GRID_HEIGHT); _j++) {
              //已经被mock数据生成了
              if (this.cells[_i][_j].type != null) {
                continue;
              }
              ...
              while (flag) {
                ...
                this.cells[_i][_j].init(this.getRandomCellType()); //cellCreateType 中随机取


                var result = this.checkPoint(_j, _i)[0];
        ...
        */
        ...
        checkPoint(x, y, recursive) {
          ...
          var rowResult = this.checkWithDirection(x, y, [v2(1, 0), v2(-1, 0)]);
          var colResult = this.checkWithDirection(x, y, [v2(0, -1), v2(0, 1)]);
          var samePoints = [];
          var newCellStatus = 0;
          ...
          var result = [samePoints, newCellStatus, this.cells[y][x].type, v2(x, y)]; // 检查一下消除的其他节点， 能不能生成更大范围的消除
        ...
        setCellTypeNum(num) {
          ...
          for (var _i2 = 0; _i2 < createTypeList.length; _i2++) {
            var index = Math.floor(Math.random() * ((_crd && CELL_BASENUM === void 0 ? (_reportPossibleCrUseOfCELL_BASENUM({
              error: Error()
            }), CELL_BASENUM) : CELL_BASENUM) - _i2)) + _i2;
              ...
              if (model.status == (_crd && CELL_STATUS === void 0 ? (_reportPossibleCrUseOfCELL_STATUS({
                ...
                for (var _i3 = 1; _i3 <= (_crd && GRID_HEIGHT === void 0 ? (_reportPossibleCrUseOfGRID_HEIGHT({
                  ...
                  if (this.cells[_i3][model.x]) {
                    if (this.cells[_i3][model.x].status != (_crd && CELL_STATUS === void 0 ? (_reportPossibleCrUseOfCELL_STATUS({
                      error: Error()
                    }), CELL_STATUS) : CELL_STATUS).COMMON) {
                ...
                for (var _i4 = 1; _i4 <= (_crd && GRID_HEIGHT === void 0 ? (_reportPossibleCrUseOfGRID_HEIGHT({
                  ...
                  for (var j = 1; j <= (_crd && GRID_WIDTH === void 0 ? (_reportPossibleCrUseOfGRID_WIDTH({
                    ...
                    if (this.cells[_i4][j] && delta <= 2) {
                      if (this.cells[_i4][j].status != (_crd && CELL_STATUS === void 0 ? (_reportPossibleCrUseOfCELL_STATUS({
                        error: Error()
                      }), CELL_STATUS) : CELL_STATUS).COMMON) {
                ...
                for (var _i5 = 1; _i5 <= (_crd && GRID_HEIGHT === void 0 ? (_reportPossibleCrUseOfGRID_HEIGHT({
                  error: Error()
                }), GRID_HEIGHT) : GRID_HEIGHT); _i5++) {
                  for (var _j2 = 1; _j2 <= (_crd && GRID_WIDTH === void 0 ? (_reportPossibleCrUseOfGRID_WIDTH({
                    error: Error()
                  }), GRID_WIDTH) : GRID_WIDTH); _j2++) {
                    if (this.cells[_i5][_j2] && this.cells[_i5][_j2].type == crushType) {
                      if (this.cells[_i5][_j2].status != (_crd && CELL_STATUS === void 0 ? (_reportPossibleCrUseOfCELL_STATUS({
                        error: Error()
                      }), CELL_STATUS) : CELL_STATUS).COMMON) {
